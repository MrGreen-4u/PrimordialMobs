...
public class GrottoceratopsModel extends AdvancedEntityModel<GrottoceratopsEntity> {
    ...
    private void setupAnimForAnimation(GrottoceratopsEntity entity, Animation animation, float limbSwing, float limbSwingAmount, float ageInTicks) {
        ...
        if (chewing) {
            float animationIntensity = PMMath.cullAnimationTick(entity.getAnimationTick(), 3, animation, partialTick, animation == GrottoceratopsEntity.ANIMATION_CHEW_FROM_GROUND ? 15 : 0);
            float jawDown = Math.min(0, PMMath.walkValue(ageInTicks, animationIntensity, 0.4F, 2F, 1F, true));
            this.head.rotateAngleX += PMMath.walkValue(ageInTicks, animationIntensity, 0.4F, 2F, 0.05F, false);
            this.jaw.rotationPointX += PMMath.walkValue(ageInTicks, animationIntensity, 0.4F, 0.5F, 2F, true);
            this.jaw.rotationPointY -= jawDown;
            this.grassBunch.rotationPointY -= jawDown * 0.5F;
            this.grassBunch2.rotationPointY -= jawDown * 0.5F;
            this.grassBunch.rotateAngleZ -= PMMath.walkValue(ageInTicks, animationIntensity, 0.4F, 2F, 0.3F, true);
            this.grassBunch2.rotateAngleZ -= PMMath.walkValue(ageInTicks, animationIntensity, 0.4F, 2F, 0.3F, false);
        }
    }
    ...
    public void setupAnim(GrottoceratopsEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (sitAmount > 0.0F) {
            ...
            //    (FK belly front & back both ~0.0 world);
            //  - the FRONT legs are a single rigid piece (no wrist). They LEAN FORWARD ("\", rotX -30)
            //    and the SHOULDER ANCHOR is RAISED into the chest (posY -10.25 => anchor at worldY
            //    ~1.39) so the leaning leg reaches the ground and the hoof rests on it (FK sole ~0.0,
            //    nothing buried) while the forward lean carries the front hoof spur OUT in front of the
            //    chest (FK toe at model z ~-27.5, well ahead of the body's front face at -17.5), fixing
            //    the spur that used to be embedded in the chest. NOTE: a rigid forward-leaning leg must
            //    tilt the sole heel-down / spur-up (the leg has no wrist), so the hoof rests on its back
            //    and the front spur lifts ~0.34 -- it's exposed in front but not itself the contact
            //    point; resting the spur flat would require a vertical leg or an added wrist bone;
            //  - the BACK legs are clearly knee-flexed (">"), soles kept FLAT on the ground
            //    (level body => flat <=> thigh + shin + foot = 0).
            progressPositionPrev(body, sitAmount, 0, 11, 0, 1F);
            // front legs: rigid, leaning forward "\", shoulder anchored high, hoof resting on the ground.
            ...
            // Back legs, knee-flexed, soles FLAT. MODEL ASYMMETRY: rfoot is parented to the shin
            // (rleg2) but lfoot is parented to the THIGH (lleg) -- so the right leg has a real
            // thigh-shin-foot knee, while the left foot hangs straight off the thigh and lleg2 is only
            // a cosmetic shin. Each side is solved independently for a flat, grounded sole:
            //  right: thigh(-44) + shin(46) + foot(-2) = 0  (real knee, flex 46 deg)
            //  left : thigh(-18) + foot(18)            = 0  (lleg2 +20 = cosmetic knee, matches right shin)
            // The left thigh must sit more upright than the right to land its (thigh-borne) foot in the
            // same place, so the left knee reads as a milder fold -- an unavoidable model asymmetry.
        ...
        float bodyBob = PMMath.walkValue(limbSwing, limbSwingAmount, walkSpeed * 1.5F, 0.5F, 2.4F, true);
        this.body.rotationPointY += bodyBob;
        this.walk(larm, walkSpeed, walkDegree * 0.4F, true, 0F, 0F, limbSwing, limbSwingAmount);
        larm.rotationPointY += Math.min(0, PMMath.walkValue(limbSwing, limbSwingAmount, walkSpeed, -1.5F, 5F, false)) - bodyBob;
        larm.rotationPointZ += PMMath.walkValue(limbSwing, limbSwingAmount, walkSpeed, -1.5F, 1F, false);
        this.walk(rarm, walkSpeed, walkDegree * 0.4F, false, 0F, 0F, limbSwing, limbSwingAmount);
        rarm.rotationPointY += Math.min(0, PMMath.walkValue(limbSwing, limbSwingAmount, walkSpeed, -1.5F, 5F, true)) - bodyBob;
        rarm.rotationPointZ += PMMath.walkValue(limbSwing, limbSwingAmount, walkSpeed, -1.5F, 1F, true);
        ...
        lleg.rotationPointY += Math.min(0, PMMath.walkValue(limbSwing, limbSwingAmount, walkSpeed, -0.5F, 5F, true)) - bodyBob;
        lleg.rotationPointZ += PMMath.walkValue(limbSwing, limbSwingAmount, walkSpeed, -0.5F, 1F, true);
        ...
        rleg.rotationPointY += Math.min(0, PMMath.walkValue(limbSwing, limbSwingAmount, walkSpeed, -0.5F, 5F, false)) - bodyBob;
        rleg.rotationPointZ += PMMath.walkValue(limbSwing, limbSwingAmount, walkSpeed, -0.5F, 1F, false);
        ...
        neck.rotationPointY += PMMath.walkValue(ageInTicks, danceAmount, danceSpeed * 1.5F, 1F, 1.5F, false);
        neck.rotationPointX += PMMath.walkValue(ageInTicks, danceAmount, danceSpeed * 1.5F, 0F, 3F, false);
