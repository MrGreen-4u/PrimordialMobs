...
public class RelicheirusModel extends AdvancedEntityModel<RelicheirusEntity> {
    ...
    private void setupAnimForAnimation(RelicheirusEntity entity, Animation animation, float limbSwing, float bodyDown, float ageInTicks) {
        ...
        if (animation == RelicheirusEntity.ANIMATION_SHAKE) {
            float animationIntensity = PMMath.cullAnimationTick(entity.getAnimationTick(), 3, animation, partialTick, 0);
        ...
        if (animation == RelicheirusEntity.ANIMATION_EAT_TREE || animation == RelicheirusEntity.ANIMATION_EAT_TRILOCARIS) {
            ...
            if (animation == RelicheirusEntity.ANIMATION_EAT_TRILOCARIS) {
                animationIntensity = PMMath.cullAnimationTick(entity.getAnimationTick(), 2, animation, partialTick, 15, 30);
            } else {
                animationIntensity = PMMath.cullAnimationTick(entity.getAnimationTick(), 3, animation, partialTick, 0);
            }
    ...
    public void setupAnim(RelicheirusEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        // Slow the walk cadence so the gait reads calm rather than frantic: ~0.72x normally, and
        // ~0.38x while ridden. This scales the leg/arm/body cadence together; the rider's vertical
        // bob in RelicheirusEntity#positionRider uses the same ridden factor (0.8*0.38=0.304) so it
        // stays in sync.
        float walkSpeed = 0.8F * (entity.isVehicle() ? 0.38F : 0.72F);
        float walkDegree = 2F;
        ...
        if (sitAmount > 0.0F) {
            // Resting theropod sit, solved with forward kinematics so the soles land FLAT on the
            // ground (the three X rotations of the leg sum to 0 => no foot tilt) and the body drop is
            // matched to this mob's (shorter) legs so the feet are planted, not buried.
            ...
            // Arms rest forward with the hand HANGING straight down (wrist reads "|", not a flat
            // "_" foot): the forearm lays low and forward from the shoulder, and at the wrist the
            // hand+claws turn to point STRAIGHT DOWN, claws resting on the ground. FK-solved
            // (chest +6, arm -105, hand +9): the hand's finger axis maps to world-down (accumulated
            // X = -90), so the claws point down (vertical wrist), and the whole arm's lowest point
            // (forearm, palm AND claws) sits at ~0 -- nothing buried. NOTE: the arm is only two rigid
            // bones and is longer than the shoulder height, so the forearm cannot also stand vertical
            // without burying; it lays forward as the ramp the hand hangs from.
