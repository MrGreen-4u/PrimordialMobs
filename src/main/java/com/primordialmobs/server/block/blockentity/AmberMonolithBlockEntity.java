...
/**
 * The Amber Monolith: shows a slowly rotating hologram of the next animal it will summon, and summons it
 * near the block once its timer elapses and a player is within 28 blocks. Faithful port of the upstream
 * Alex's Caves block entity, with two standalone adaptations:
 * - the primordial-boss "migration" logic (Atlatitan, ACWorldData) is gone with the boss itself;
 * - vanilla biomes carry no "alexscaves:cave_creature" spawn entries, so when the biome list is empty the
 *   monolith draws from the mod's own dinosaur table, weighted exactly like the original Primordial Caves
 *   biome (grottoceratops 27/2-4, relicheirus 13/1-1, subterranodon 6/3-5, vallumraptor 6/6-7,
 *   tremorsaurus 5/1-1).
 */
public class AmberMonolithBlockEntity extends BlockEntity {
    ...
    public AmberMonolithBlockEntity(BlockPos pos, BlockState state) {
        super(PMBlockEntityRegistry.AMBER_MONOLITH.get(), pos, state);
    }
            ...
            if (entity.spawnsMobIn <= 0) {
                if (entity.spawnType == null) {
                    ...
                    if (level.getNearestPlayer(entity.getBlockPos().getX() + 0.5F, entity.getBlockPos().getY() + 0.5F, entity.getBlockPos().getZ() + 0.5F, 28, false) != null && entity.spawnMobs()) {
                        level.playSound((Player) null, blockPos, PMSoundRegistry.AMBER_MONOLITH_SUMMON.get(), SoundSource.BLOCKS);
                        entity.generateSpawnData();
                    }
                }
            }
            ...
            for (int i2 = 0; !flag && i2 < 6; i2++) {
                ...
                if (spawnType.canSummon() && NaturalSpawner.isSpawnPositionOk(SpawnPlacements.getPlacementType(spawnType), level, blockpos, spawnType)) {
                    ...
                    try {
                        ...
                        PrimordialMobs.LOGGER.warn("Failed to create mob", (Throwable) exception);
                        continue;
                    }
                    ...
                    if (entity instanceof Mob) {
                        ...
                        if (net.minecraftforge.event.ForgeEventFactory.checkSpawnPosition(mob, (ServerLevelAccessor) level, MobSpawnType.CHUNK_GENERATION)) {
                            ...
                            for (int i = 0; i < maxDist; i++) {
                                ...
                                ((ServerLevel) level).sendParticles(PMParticleRegistry.AMBER_MONOLITH.get(), vec3.x, vec3.y, vec3.z, 0, target.x, target.y, target.z, 1D);
                            }
                            for (int i = 0; i < 5; i++) {
                                ((ServerLevel) level).sendParticles(PMParticleRegistry.AMBER_EXPLOSION.get(), entity.getRandomX(1.0F), entity.getRandomY(), entity.getRandomZ(1.0F), 0, 0, 0, 0, 1D);
                            }
                        }
                    }
                }
    ...
    private BlockPos getRandomSpawnPos() {
        boolean caveCreature = spawnType.getCategory() == PMEntityRegistry.CAVE_CREATURE;
    ...
    private void generateSpawnData() {
        ...
        int i = Math.max(1000, PrimordialMobs.COMMON_CONFIG.amberMonolithMeanTime.get());
    ...
    private static boolean isEntitySpawnBlocked(MobSpawnSettings.SpawnerData settings, Level level, BlockPos pos, int range) {
        if (settings.type.is(PMTagRegistry.AMBER_MONOLITH_SKIPS)) {
            return true;
        }
    ...
    /**
     * The mod's own dinosaur table, used when the biome has no cave_creature spawn entries (i.e. always,
     * standalone). Weights copied from the upstream alexscaves:primordial_caves biome definition.
     */
    private static WeightedRandomList<MobSpawnSettings.SpawnerData> builtinDinosaurSpawns() {
        return WeightedRandomList.create(
                new MobSpawnSettings.SpawnerData(PMEntityRegistry.GROTTOCERATOPS.get(), 27, 2, 4),
                new MobSpawnSettings.SpawnerData(PMEntityRegistry.RELICHEIRUS.get(), 13, 1, 1),
                new MobSpawnSettings.SpawnerData(PMEntityRegistry.SUBTERRANODON.get(), 6, 3, 5),
                new MobSpawnSettings.SpawnerData(PMEntityRegistry.VALLUMRAPTOR.get(), 6, 6, 7),
                new MobSpawnSettings.SpawnerData(PMEntityRegistry.TREMORSAURUS.get(), 5, 1, 1));
    }

    private static MobSpawnSettings.SpawnerData getEntitySpawnSettingsForBiome(Level level, BlockPos pos) {
        Biome biome = level.getBiome(pos).value();
        if (biome != null) {
            WeightedRandomList<MobSpawnSettings.SpawnerData> spawnList = biome.getMobSettings().getMobs(PMEntityRegistry.CAVE_CREATURE);
            if (spawnList.isEmpty()) {
                spawnList = builtinDinosaurSpawns();
            }
            if (!spawnList.isEmpty()) {
                return spawnList.getRandom(level.random).orElse(null);
            }
        }
        return null;
    }
