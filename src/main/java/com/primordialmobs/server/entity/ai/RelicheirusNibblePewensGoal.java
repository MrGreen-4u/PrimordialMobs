...
public class RelicheirusNibblePewensGoal extends MoveToBlockGoal {
    ...
    private int getHeightOfBlock(LevelReader worldIn, BlockPos pos) {
        int i = 0;
        while (pos.getY() > worldIn.getMinBuildHeight() && (worldIn.getBlockState(pos).is(PMTagRegistry.RELICHEIRUS_NIBBLES) || worldIn.getBlockState(pos).isAir() || worldIn.getBlockState(pos).is(PMTagRegistry.RELICHEIRUS_KNOCKABLE_LOGS))) {
            pos = pos.below();
            i++;
        }
        return i;
    ...
    protected boolean isValidTarget(LevelReader worldIn, BlockPos pos) {
        return worldIn.getBlockState(pos).is(PMTagRegistry.RELICHEIRUS_NIBBLES) && highEnough(worldIn, pos);
    }
