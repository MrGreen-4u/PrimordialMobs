...
public class DinosaurSpiritEntity extends Entity {
    ...
    public DinosaurSpiritEntity(PlayMessages.SpawnEntity spawnEntity, Level level) {
        this(PMEntityRegistry.DINOSAUR_SPIRIT.get(), level);
    }
    ...
    private void tickGrottoceratops(Player player) {
        ...
        this.noPhysics = true;
        if(!level().isClientSide && !player.getUseItem().is(PMItemRegistry.EXTINCTION_SPEAR.get())){
            this.setFading(true);
        }
    }
    ...
    private void tickTremorsaurus(Player player) {
        ...
        if (target != null) {
            ...
            if (inRange && this.abilityProgress >= 5) {
                if(!dealtDamage && target.hurt(PMDamageTypes.causeSpiritDinosaurDamage(level().registryAccess(), player), 3 + 2 * getEnchantmentLevel())){
                    dealtDamage = true;
                }
                this.setFading(true);
            }
