...
import com.primordialmobs.server.entity.util.LaysEggs;
...
public abstract class DinosaurEntity extends TamableAnimal implements IDancesToJukebox, LaysEggs, IAdvancedPathingMob, RidingMeterMount {
    ...
    private static final EntityDataAccessor<Boolean> RECOLORED = SynchedEntityData.defineId(DinosaurEntity.class, EntityDataSerializers.BOOLEAN);
    ...
    private boolean sittingDimensions;
    ...
    protected void defineSynchedData() {
        ...
        this.entityData.define(RECOLORED, false);
    }
    ...
    public static boolean checkPrehistoricPostBossSpawnRules(EntityType<? extends Animal> type, LevelAccessor levelAccessor, MobSpawnType mobType, BlockPos pos, RandomSource randomSource) {
        return checkPrehistoricSpawnRules(type, levelAccessor, mobType, pos, randomSource);
    }
    ...
    public void tick() {
        ...
        // A seated dinosaur must never drift: zero any residual or push-induced horizontal motion and
        // halt navigation so it stays put (travel() only cancels AI movement input, not momentum).
        ...
        // Shrink the collision box to follow the lowered body while sitting, so the hitbox no longer
        // floats at full standing height above a crouched dinosaur.
        if (this.sittingDimensions != this.isInSittingPose()) {
            this.sittingDimensions = this.isInSittingPose();
            this.refreshDimensions();
        }
    }

    // Per-species multiplier applied to the bounding box height while sitting (1.0 = unchanged).
    ...
    public EntityDimensions getDimensions(Pose pose) {
        ...
        float scale = this.getSittingDimensionScale();
        if (scale != 1.0F && this.isInSittingPose()) {
            return EntityDimensions.scalable(dimensions.width, dimensions.height * scale);
        }
        return dimensions;
    }
    ...
    public boolean isRecolored() {
        return this.entityData.get(RECOLORED);
    }

    public void setRecolored(boolean recolored) {
        this.entityData.set(RECOLORED, recolored);
    }
    ...
    public void addAdditionalSaveData(CompoundTag compound) {
        ...
        compound.putBoolean("Recolored", this.isRecolored());
    }
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        ...
        this.setRecolored(compound.getBoolean("Recolored"));
        //compatibility with the old ember Logger encoding (AltSkin 3 = recolored standard skin)
        if (altSkin == 3) {
            altSkin = 0;
    ...
    public void handleEntityEvent(byte b) {
        if (b == 77) {
            ...
            for (int i1 = 0; i1 < particleCount; i1++) {
                ...
                BlockPos ground = BlockPos.containing(PMMath.getGroundBelowPosition(level(), new Vec3(Mth.floor(this.getX() + extraX), Mth.floor(this.getY() + extraY), Mth.floor(this.getZ() + extraZ))));
            ...
            // Skin-swap orbiting particle trails (amber/tectonic) intentionally removed; the alternate
            // texture still changes server-side via setAltSkin, just without the swirling effect.
        } else {
    ...
    public int getAltSkinForItem(ItemStack stack) {
        if (stack.is(PMItemRegistry.AMBER_CURIOSITY.get())) {
            return 1;
        } else if (stack.is(PMItemRegistry.TECTONIC_SHARD.get())) {
            return 2;
        } else {
            return 0;
        }
    ...
    public BlockState createEggBeddingBlockState() {
        return PMBlockRegistry.FERN_THATCH.get().defaultBlockState();
    }
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (!interactionresult.consumesAction() && !type.consumesAction()) {
            ...
            if (altSkinFromItem > 0) {
                ...
                this.playSound(altSkinFromItem == 2 ? PMSoundRegistry.TECTONIC_SHARD_TRANSFORM.get() : PMSoundRegistry.AMBER_MONOLITH_SUMMON.get());
                if (!level().isClientSide) {
            ...
            if (nearbyDinosaurEntityTypes.size() >= 5) {
                for (Player player : level().getEntitiesOfClass(Player.class, this.getBoundingBox().inflate(advancementRange))) {
                    if (player.distanceTo(this) < advancementRange) {
                        PMAdvancementTriggerRegistry.DINOSAURS_MINECART.triggerForEntity(player);
                    }
                }
