...
import com.primordialmobs.server.block.MultipleDinosaurEggsBlock;
...
public class SubterranodonEntity extends DinosaurEntity implements PackAnimal, FlyingAnimal, KeybindUsingMount, FlyingMount {
    ...
    public static boolean checkSubterranodonSpawnRules(EntityType<? extends Animal> type, LevelAccessor levelAccessor, MobSpawnType mobType, BlockPos pos, RandomSource randomSource) {
        ...
        return (below.is(PMTagRegistry.DINOSAURS_SPAWNABLE_ON) || below.is(PMBlockRegistry.PEWEN_BRANCH.get()) || below.is(BlockTags.LEAVES)) && levelAccessor.getFluidState(pos).isEmpty() && levelAccessor.getFluidState(pos.below()).isEmpty();
    }
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(6, new TemptGoal(this, 1.1D, Ingredient.of(Items.COD, Items.COOKED_COD, PMItemRegistry.COOKED_TRILOCARIS_TAIL.get(), PMItemRegistry.TRILOCARIS_TAIL.get()), false));
    ...
    public void tick() {
        ...
        if (isFlying()) {
            if(timeFlying % 10 == 0 && (flapAmount > 0 || controlUpTicks > 0)){
                this.playSound(PMSoundRegistry.SUBTERRANODON_FLAP.get());
            }
            timeFlying++;
        ...
        if (!level().isClientSide) {
            ...
            if (player != null && player.isPassengerOfSameVehicle(this)) {
                if (PrimordialMobs.PROXY.isKeyDown(0) && !PrimordialMobs.PROXY.isKeyDown(1) && controlUpTicks < 2 && getMeterAmount() > 0.1F) {
                    if (getMeterAmount() > 0.1F) {
                        PrimordialMobs.sendMSGToServer(new MountedEntityKeyMessage(this.getId(), player.getId(), 0));
                        controlUpTicks = 5;
                    }
                }
                if (PrimordialMobs.PROXY.isKeyDown(1) && !PrimordialMobs.PROXY.isKeyDown(0) && controlDownTicks < 2) {
                    PrimordialMobs.sendMSGToServer(new MountedEntityKeyMessage(this.getId(), player.getId(), 1));
                    controlDownTicks = 5;
                }
            }
        }
        ...
        if (this.entityData.get(ATTACK_TICK) > 0) {
            ...
            if (attackProgress == 5F && target != null && this.distanceTo(target) < 3D + target.getBbWidth() && this.hasLineOfSight(target)) {
                ...
                this.playSound(PMSoundRegistry.SUBTERRANODON_ATTACK.get());
            }
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob mob) {
        return PMEntityRegistry.SUBTERRANODON.get().create(serverLevel);
    }
    ...
    public BlockState createEggBlockState() {
        return PMBlockRegistry.SUBTERRANODON_EGG.get().defaultBlockState().setValue(MultipleDinosaurEggsBlock.EGGS, 1 + random.nextInt(3));
    }
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (prev != InteractionResult.SUCCESS) {
            ...
            if (!this.isTame() && (itemStack.is(PMItemRegistry.TRILOCARIS_TAIL.get()) || itemStack.is(PMItemRegistry.COOKED_TRILOCARIS_TAIL.get()))) {
    ...
    protected SoundEvent getAmbientSound() {
        return PMSoundRegistry.SUBTERRANODON_IDLE.get();
    }

    protected SoundEvent getHurtSound(DamageSource damageSource) {
        return PMSoundRegistry.SUBTERRANODON_HURT.get();
    }

    protected SoundEvent getDeathSound() {
        return PMSoundRegistry.SUBTERRANODON_DEATH.get();
    }
