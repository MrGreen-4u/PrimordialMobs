...
import com.primordialmobs.server.entity.util.KeybindUsingMount;
...
public class TremorsaurusEntity extends DinosaurEntity implements KeybindUsingMount, IAnimatedEntity, ShakesScreen, ITallWalker {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(6, new TemptGoal(this, 1.1D, Ingredient.of(PMBlockRegistry.COOKED_DINOSAUR_CHOP.get(), PMBlockRegistry.DINOSAUR_CHOP.get()), false));
    ...
    public void tick() {
        ...
        if (this.onGround() && !this.isInFluidType() && this.walkAnimation.speed() > 0.1F && !this.isBaby()) {
            ...
            if (Math.abs(f) < 0.2) {
                if (screenShakeAmount <= 0.3) {
                    this.playSound(PMSoundRegistry.TREMORSAURUS_STOMP.get(), 2, 1.0F);
                    this.shakeWater();
                }
        ...
        if (!level().isClientSide) {
            ...
            if (!held && getHeldMobId() != -1) {
                this.setHeldMobId(-1);
                this.playSound(PMSoundRegistry.TREMORSAURUS_THROW.get());
                riderHitEntity = null;
            }
        } else {
            Player player = PrimordialMobs.PROXY.getClientSidePlayer();
            if (player != null && player.isPassengerOfSameVehicle(this)) {
                if (PrimordialMobs.PROXY.isKeyDown(2) && getMeterAmount() >= 1.0F) {
                    PrimordialMobs.sendMSGToServer(new MountedEntityKeyMessage(this.getId(), player.getId(), 2));
                }
                if (PrimordialMobs.PROXY.isKeyDown(3) && (this.getAnimation() == NO_ANIMATION || this.getAnimation() == null)) {
                    PrimordialMobs.sendMSGToServer(new MountedEntityKeyMessage(this.getId(), player.getId(), 3));
                }
            }
        }
        if (this.isVehicle()) {
    ...
    private void playRoarSound() {
        if(this.isBaby()){
            this.playSound(PMSoundRegistry.TREMORSAURUS_ROAR.get(), 1.0F, 1.5F);
        }else{
            this.playSound(PMSoundRegistry.TREMORSAURUS_ROAR.get(), 4.0F, 1.0F);
        }
    }
    ...
    private void scareMobs() {
        ...
        for (LivingEntity e : list) {
            if (!e.getType().is(PMTagRegistry.RESISTS_TREMORSAURUS_ROAR) && !isAlliedTo(e)) {
            ...
            for (int x = -radius; x <= radius; x++) {
                for (int z = -radius; z <= radius; z++) {
                    if (x * x + z * z <= radius * radius) {
                        ...
                        if (water > 0.0F) {
                            level().addParticle(PMParticleRegistry.WATER_TREMOR.get(), mutableBlockPos.getX() + 0.5F, mutableBlockPos.getY() + water + 0.01, mutableBlockPos.getZ() + 0.5F, 0, 0, 0);
                        }
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob mob) {
        return PMEntityRegistry.TREMORSAURUS.get().create(level);
    }

    @Override
    public boolean wantsToAttack(LivingEntity living, LivingEntity owner) {
        if(living instanceof TremorsaurusEntity tremorsaurus && (tremorsaurus.getTameAttempts() > 0 || tremorsaurus.hasEffect(PMEffectRegistry.STUNNED.get()))){
            return false;
        }
    ...
    public BlockState createEggBlockState() {
        return PMBlockRegistry.TREMORSAURUS_EGG.get().defaultBlockState();
    }
    ...
    public void positionRider(Entity passenger, MoveFunction moveFunction) {
        if (this.isPassengerOfSameVehicle(passenger) && passenger instanceof LivingEntity living && !this.touchingUnloadedChunk()) {
            ...
            float maxLegSolverHeight = (1F - PMMath.smin(1F - heightBackLeft, 1F - heightBackRight, 0.1F)) * 0.8F;
    ...
    public boolean onFeedMixture(ItemStack itemStack, Player player) {
        if (itemStack.is(PMItemRegistry.SERENE_SALAD.get()) && this.hasEffect(PMEffectRegistry.STUNNED.get())) {
            this.removeEffect(PMEffectRegistry.STUNNED.get());
            PrimordialMobs.sendMSGToAll(new UpdateEffectVisualityEntityMessage(this.getId(), this.getId(), 3, 0, true));
            if (!level().isClientSide) {
    ...
    public boolean isFood(ItemStack stack) {
        return this.isTame() && (stack.is(PMBlockRegistry.COOKED_DINOSAUR_CHOP.get().asItem()) || stack.is(PMBlockRegistry.DINOSAUR_CHOP.get().asItem()));
    }
    ...
    protected SoundEvent getAmbientSound() {
        return PMSoundRegistry.TREMORSAURUS_IDLE.get();
    }

    protected SoundEvent getHurtSound(DamageSource damageSource) {
        return PMSoundRegistry.TREMORSAURUS_HURT.get();
    }

    protected SoundEvent getDeathSound() {
        return PMSoundRegistry.TREMORSAURUS_DEATH.get();
    }
