...
import com.primordialmobs.server.entity.util.ChestThief;
import com.primordialmobs.server.entity.util.PackAnimal;
import com.primordialmobs.server.entity.util.TargetsDroppedItems;
...
public class VallumraptorEntity extends DinosaurEntity implements IAnimatedEntity, ICustomCollisions, PackAnimal, ChestThief, TargetsDroppedItems {
    ...
    private static final Predicate<LivingEntity> VALLUMRAPTOR_TARGETS = living -> living.getType().is(PMTagRegistry.VALLUMRAPTOR_TARGETS);
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(7, new TemptGoal(this, 1.1D, Ingredient.of(PMItemRegistry.DINOSAUR_NUGGET.get()), false));
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob mob) {
        return PMEntityRegistry.VALLUMRAPTOR.get().create(level);
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (eatHeldItemIn > 0) {
                ...
                this.heal(5);
                if (stack.is(PMItemRegistry.DINOSAUR_NUGGET.get()) && justLootedChest) {
    ...
    public void actuallyPlayAmbientSound() {
        ...
        if (this.getAnimation() == ANIMATION_CALL_2) {
            soundevent = PMSoundRegistry.VALLUMRAPTOR_CALL.get();
            volume += 1.0F;
        }
        if (soundevent != null) {
    ...
    public void afterSteal(BlockPos stealPos) {
        ...
        if (this.getItemInHand(InteractionHand.MAIN_HAND).is(PMItemRegistry.DINOSAUR_NUGGET.get())) {
    ...
    public boolean canTargetItem(ItemStack stack) {
        return (stack.is(PMTagRegistry.VALLUMRAPTOR_STEALS) || stack.getItem().isEdible() && stack.getItem().getFoodProperties(stack, this).isMeat()) && !stack.is(PMBlockRegistry.VALLUMRAPTOR_EGG.get().asItem());
    }
    ...
    public BlockState createEggBlockState() {
        return PMBlockRegistry.VALLUMRAPTOR_EGG.get().defaultBlockState().setValue(MultipleDinosaurEggsBlock.EGGS, 1 + random.nextInt(3));
    }
    ...
    public boolean isFood(ItemStack stack) {
        return this.isTame() && stack.is(PMItemRegistry.DINOSAUR_NUGGET.get());
    }
    ...
    protected SoundEvent getAmbientSound() {
        return this.getRelaxedFor() > 0 ? PMSoundRegistry.VALLUMRAPTOR_SLEEP.get() : PMSoundRegistry.VALLUMRAPTOR_IDLE.get();
    }

    protected SoundEvent getHurtSound(DamageSource damageSource) {
        return PMSoundRegistry.VALLUMRAPTOR_HURT.get();
    }

    protected SoundEvent getDeathSound() {
        return PMSoundRegistry.VALLUMRAPTOR_DEATH.get();
    }


    @Override
    public boolean onFeedMixture(ItemStack itemStack, Player player) {
        if (itemStack.is(PMItemRegistry.SERENE_SALAD.get()) && this.getRelaxedFor() > 0 && !this.isTame()) {
            this.heal(5);
