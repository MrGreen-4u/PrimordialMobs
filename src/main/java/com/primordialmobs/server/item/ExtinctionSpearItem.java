...
import com.primordialmobs.server.entity.item.ExtinctionSpearEntity;
...
public class ExtinctionSpearItem extends SpearItem {
    ...
    public void releaseUsing(ItemStack itemStack, Level level, LivingEntity livingEntity, int i1) {
        if (livingEntity instanceof Player player) {
            ...
            if (f > 0.1D) {
                ...
                level.playSound((Player) null, spearEntity, PMSoundRegistry.EXTINCTION_SPEAR_THROW.get(), SoundSource.PLAYERS, 1.0F, 1.0F);
                if (!player.getAbilities().instabuild) {
                    itemStack.shrink(1);
                }
    ...
    public boolean hurtEnemy(ItemStack stack, LivingEntity hurtEntity, LivingEntity player) {
        ...
        DinosaurSpiritEntity dinosaurSpirit = PMEntityRegistry.DINOSAUR_SPIRIT.get().create(player.level());
    ...
    public boolean isValidRepairItem(ItemStack item, ItemStack repairItem) {
        return repairItem.is(PMItemRegistry.TECTONIC_SHARD.get()) || super.isValidRepairItem(item, repairItem);
    }
}
