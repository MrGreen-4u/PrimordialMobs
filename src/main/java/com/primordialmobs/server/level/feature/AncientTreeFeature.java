...
import com.primordialmobs.server.block.TreeStarBlock;
...
public class AncientTreeFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        for (Direction direction : Direction.values()) {
            if (direction.getAxis() != Direction.Axis.Y) {
                ...
                drawLeafOrb(level, canopyPos.relative(direction, branchOut).above(branchUp + 2), randomsource, PMBlockRegistry.ANCIENT_LEAVES.get().defaultBlockState(), 2 + randomsource.nextInt(2), 4 + randomsource.nextInt(2), 2 + randomsource.nextInt(2));
            }
        }
                        ...
                        if (canReplace(level.getBlockState(fill))) {
                            ...
                            if (random.nextInt(5) == 0) {
                                ...
                                if (level.getBlockState(starPos).isAir()) {
                                    level.setBlock(starPos, PMBlockRegistry.TREE_STAR.get().defaultBlockState().setValue(TreeStarBlock.FACING, dir), 3);
                                }
                            }
                        }
                    }
    ...
    private static boolean canReplace(BlockState state) {
        return (state.isAir() || state.canBeReplaced() || state.is(PMBlockRegistry.ANCIENT_LEAVES.get()) || state.is(PMBlockRegistry.TREE_STAR.get())) && !state.is(PMTagRegistry.UNMOVEABLE) && state.getFluidState().isEmpty();
    }
