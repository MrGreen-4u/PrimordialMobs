...
public class PewenTreeFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        for (int i = 0; i <= height; i++) {
            ...
            level.setBlock(trunkPos, i == height ? PMBlockRegistry.PEWEN_WOOD.get().defaultBlockState() : PMBlockRegistry.PEWEN_LOG.get().defaultBlockState(), 3);
            if (i == height) {
                level.setBlock(trunkPos.above(), PMBlockRegistry.PEWEN_PINES.get().defaultBlockState(), 3);
            }
            if (i > j) {
                ...
                if (canReplace(level.getBlockState(offset))) {
                    level.setBlock(offset, PMBlockRegistry.PEWEN_BRANCH.get().defaultBlockState().setValue(PewenBranchBlock.ROTATION, branchRot).setValue(PewenBranchBlock.PINES, true), 3);
                }
            }
        }
        return true;
    ...
    private void buildPenumbra(WorldGenLevel level, BlockPos trunkPos, int currentIndex, int maxIndex, RandomSource randomSource) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            for (int j = 1; j <= actualLength; j++) {
                ...
                if (canReplace(level.getBlockState(offset))) {
                    level.setBlock(offset, PMBlockRegistry.PEWEN_BRANCH.get().defaultBlockState().setValue(PewenBranchBlock.ROTATION, i).setValue(PewenBranchBlock.PINES, j == actualLength), 3);
                }
            }
        }
    }
    ...
    private static boolean canReplace(BlockState state) {
        return (state.isAir() || state.canBeReplaced() || state.is(PMBlockRegistry.PEWEN_BRANCH.get())) && !state.is(PMTagRegistry.UNMOVEABLE) && state.getFluidState().isEmpty();
    }
}
